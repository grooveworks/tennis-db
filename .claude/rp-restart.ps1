...
$null = Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{
  CommandLine = "`"$exew`" racketpedia\listener.py"; CurrentDirectory = "D:\Downloads\Claude\tennis"
}
...
try {
  ...
  if (Test-Path racketpedia\listener.log) { Get-Content racketpedia\listener.log -Tail 8 }
}
