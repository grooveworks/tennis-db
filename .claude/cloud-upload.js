...
function extractData(file) {
  ...
  const m = h.match(/var DATA = (\[[\s\S]*?\]);\r?\n/) || h.match(/var DATA = (\[[\s\S]*?\]);/);
  if (!m) throw new Error("DATA が見つからない: " + file);
  return m[1];
}

const ROOT = path.join(__dirname, "..");
const BLOBS = {
  strings: extractData(path.join(ROOT, "racketpedia", "string_compare.html")),
  rackets: extractData(path.join(ROOT, "racketpedia", "racket_compare.html")),
  reader: extractData(path.join(ROOT, "tennisone", "reader.html")),
};
