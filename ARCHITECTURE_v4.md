...
**各 Stage で実装着手前に、以下を応答テキストに明示的に書き出すこと。書かずに着手したらルール違反＝自己停止＋ユーザー報告。**
...
**この自問を実装着手前に応答テキストに書き出すこと**（書かずに進めたらルール違反）。

---

## 11. ストップサイン（対処療法の兆候、発動したら即停止）

以下のどれかが発動したら、**その瞬間に手を止めてユーザー報告**。取り繕わない、続行しない。

### 11.1 同一ファイル 3 回目の Edit
同じファイルを短時間に何度も編集している = パッチの上のパッチ = 対処療法のサイン。
→ 止まって「このファイルに 3 回目の編集です。根本から見直すべきかもしれません」と報告。

### 11.2 エラーに対して「これで回避できる」と思った瞬間
回避策は根本原因調査をスキップしている。
→ 止まって「エラーの根本原因を調べます」と宣言してから原因特定に戻る。

### 11.3 「とりあえず」「後で直す」「一旦」の単語が出そうになった
これらは対処療法の兆候。
→ 書かない。書きそうになったら止まって、ちゃんとした解決案を考える。

### 11.4 判断をユーザーに投げそうになった
「A/B/C どちらにしますか?」「ご判断ください」「指示ください（判断材料なし）」
→ 止まって、まず自分で判断案を出す。「こうします。違えば指示ください」の形に書き換える。

### 11.5 事前調査なしに実装を始めそうになった
Stage 開始前チェックリスト（§10.4）を書かずにコードを書き始めた。
→ 止まって、チェックリストを埋めてから再開。

### 11.6 UI/UX を機能実装より後回しにしそうになった
「機能を作ってから見た目を整える」発想 = v3 の失敗パターン。
→ 止まって、デザイン一式（S2-S5）が未完であれば機能実装に進まない。

### 11.7 設計書に書かれていない機能を実装しそうになった
「ついでに」「ちょうど必要だから」で機能を増やす。
→ 止まって、REQUIREMENTS_v4.md に追記してユーザー承認を得てから実装。

### 11.8 環境で動かないことを DoD に含めた
DoD の検証手段が preview で不可能なら、DoD 設計が浅い。
→ 止まって、DoD を検証可能な形に書き直す（または「本番環境で確認」と明示）。

---

## 12. UI/UX 先行ルール（v3 の最大の反省）

### 12.1 デザインシステム完成前の機能実装禁止
- ROADMAP の S2-S5 (デザイン一式: デザインシステム / ワイヤフレーム / 共通 UI コンポーネント / Sessions タブ再設計) 完了前に、S6 以降の機能実装 Stage には進まない
- どうしても前倒しで試したくなった時 = ストップサイン §11.6 発動

### 12.2 全機能が同じデザインシステムの制約下
- 新しいコンポーネントを作る時は必ず `DESIGN_SYSTEM_v4.md` の仕様を参照
- 独自スタイルの one-off 追加を禁止
- 「この画面だけ特別」は作らない

### 12.3 ユニバーサルデザインは最初から組み込む
- 後付けではなく、デザインシステム定義の時点で組み込む
- REQUIREMENTS_v4.md §N1a 参照
