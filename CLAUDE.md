...
**テニスの議論・意思決定・ギア検討を行う時は、まず `.claude/tennis/状態.md` を読む** (2026-06-02 新設)。
そこから 現状データ(`.claude/data-latest.json`) / 決定・保留(`.claude/tennis/決定.md`) / 過去ログ書庫(`.claude/tennis/logs-tagged.json` をタグで取り出し: `node .claude/tennis/fetch.js "#タグ"`) へ辿る。
目的 = 再説明させない・前回の保留(next-action)から「あれやった?」で始める。原則 = 体感は(弦+テンション+ラケット+イベント)に紐付ける / 要約で本質を消さない / 事実と推測を区別する。
