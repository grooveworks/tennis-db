...
function computeSettingHistory(racketName, tournaments, practices, trials) {
  ...
  (tournaments || []).forEach(t => {
    ...
    // matches[] 内の racketName 一致 (matches に個別ラケット指定あり)
    (t.matches || []).forEach(m => {
      if (!m || m.racketName !== racketName) return;
      // 試合単位で別シグネチャ → 親 tournament の date を使う
      // ただし tournament 自身も racketName 一致だと重複するので、
      // tournament に racketName 一致 + matches も同じラケット → tournament 側で 1 つ
      // matches[] のラケットが tournament と異なる時のみ追加
      if (t.racketName !== racketName) {
        items.push({
          type: "tournament-match",
          date: t.date,
          sig: _settingSignature(m),
          sessionId: t.id,
          item: t,
        });
      }
