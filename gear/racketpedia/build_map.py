...
# 2.1) データ取得日バッジ: last_captured の分布からヘッダーの @@ASOF@@ トークンを差し替え (2026-07-09)
#      アプリのデータは本家の“今”ではなく取得時点のスナップショットである、と明示する。
_caps = sorted([d['tech']['cap'] for d in out if d['tech'].get('cap')])
if _caps:
    _base, _latest = _caps[0], _caps[-1]
    asof_txt = '本家データ取得: %s 時点%s' % (_base, ('（最終再取得 %s）' % _latest) if _latest != _base else '')
else:
    asof_txt = '本家データ取得日: 記録なし'
assert '@@ASOF@@' in design, 'ヘッダーの @@ASOF@@ トークンが見つからない (dc.html 未更新)'
design = design.replace('@@ASOF@@', asof_txt, 1)
print('データ取得日バッジ:', asof_txt)
